...
@section('title', 'Demografi Desa - ' . ($profil->nama_desa ?? 'Desa Tanalum'))
        ...
        <div class="container mx-auto px-4 relative z-10">
            <div class="text-center text-white">
                <nav class="flex items-center justify-center space-x-2 text-sm mb-4" data-aos="fade-down">
                    <a href="{{ route('home') }}" class="hover:text-primary-200">Beranda</a>
                    <span>/</span>
                    <a href="{{ route('profil.index') }}" class="hover:text-primary-200">Profil</a>
                    <span>/</span>
                ...
                <p class="text-lg text-primary-100 max-w-2xl mx-auto" data-aos="fade-up" data-aos-delay="100">
                    Statistik kependudukan {{ $profil->nama_desa ?? 'Desa Tanalum' }}
                </p>
            </div>
        </div>
        ...
        <div class="container mx-auto px-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
                <div class="bg-white rounded-2xl shadow-sm p-6 text-center" data-aos="fade-up">
                    ...
                    <p class="text-3xl font-bold text-gray-800">{{ number_format($statistik->total_penduduk ?? 0) }}</p>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6 text-center" data-aos="fade-up" data-aos-delay="50">
                    ...
                    <p class="text-3xl font-bold text-gray-800">{{ number_format($statistik->laki_laki ?? 0) }}</p>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6 text-center" data-aos="fade-up" data-aos-delay="100">
                    ...
                    <p class="text-3xl font-bold text-gray-800">{{ number_format($statistik->perempuan ?? 0) }}</p>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6 text-center" data-aos="fade-up" data-aos-delay="150">
                    ...
                    <p class="text-3xl font-bold text-gray-800">{{ number_format($statistik->jumlah_kk ?? 0) }}</p>
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">
                                    {{ $statistik->total_penduduk > 0 ? round(($statistik->laki_laki / $statistik->total_penduduk) * 100, 1) : 0 }}%
                                </p>
                            </div>
                        </div>
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">
                                    {{ $statistik->total_penduduk > 0 ? round(($statistik->perempuan / $statistik->total_penduduk) * 100, 1) : 0 }}%
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        @foreach ($pendidikan as $item)
                            <div>
                                <div class="flex justify-between text-sm mb-1">
                                    ...
                                    <span class="font-medium text-gray-800">{{ number_format($item['value']) }}</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-primary-500 h-2 rounded-full"
                                        style="width: {{ $totalPendidikan > 0 ? ($item['value'] / $totalPendidikan) * 100 : 0 }}%">
                                    </div>
                                </div>
                            </div>
                        @endforeach
                            ...
                            <div>
                                ...
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-amber-500 h-2 rounded-full"
                                        style="width: {{ $totalPekerjaan > 0 ? ($item['value'] / $totalPekerjaan) * 100 : 0 }}%">
                                    </div>
                                </div>
                            </div>
                        @endforeach
                            ...
                            <div>
                                ...
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-green-500 h-2 rounded-full"
                                        style="width: {{ $totalAgama > 0 ? ($item['value'] / $totalAgama) * 100 : 0 }}%">
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($dusuns as $index => $dusun)
                            <div class="bg-white rounded-2xl shadow-sm p-6" data-aos="fade-up"
                                ...
                                <h4 class="font-bold text-gray-800 mb-4">{{ $dusun->nama }}</h4>
                                <div class="space-y-3">
                                    <div class="flex justify-between">
                                        <span class="text-gray-500">Jumlah RT</span>
                                        <span class="font-semibold">{{ $dusun->jumlah_rt ?? '-' }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-gray-500">Jumlah KK</span>
                                        <span class="font-semibold">{{ number_format($dusun->jumlah_kk ?? 0) }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-gray-500">Penduduk</span>
                                        <span
                                            class="font-semibold">{{ number_format($dusun->jumlah_penduduk ?? 0) }}</span>
                                    </div>
                                </div>
                            </div>
                        @endforeach
    ...
    <script>
        ...
        new Chart(genderCtx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [{{ $statistik->laki_laki ?? 0 }}, {{ $statistik->perempuan ?? 0 }}],
        ...
        new Chart(ageCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Jumlah',
                    data: [
                        {{ $statistik->usia_0_5 ?? 0 }},
                        {{ $statistik->usia_6_12 ?? 0 }},
                        {{ $statistik->usia_13_17 ?? 0 }},
                        {{ $statistik->usia_18_25 ?? 0 }},
                        {{ $statistik->usia_26_40 ?? 0 }},
                        {{ $statistik->usia_41_60 ?? 0 }},
                        {{ $statistik->usia_60_plus ?? 0 }}
                    ],
