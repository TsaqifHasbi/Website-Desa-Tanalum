...
@section('content')
        ...
        <form action="" method="GET" class="flex flex-wrap gap-4 items-end">
            <div>
                ...
                <select name="jenis"
                    ...
                    <option value="perangkat" {{ request('jenis') == 'perangkat' ? 'selected' : '' }}>Perangkat Desa
                    </option>
                    <option value="bpd" {{ request('jenis') == 'bpd' ? 'selected' : '' }}>BPD</option>
                    <option value="lpm" {{ request('jenis') == 'lpm' ? 'selected' : '' }}>LPM</option>
                    <option value="pkk" {{ request('jenis') == 'pkk' ? 'selected' : '' }}>PKK</option>
                    <option value="karang_taruna" {{ request('jenis') == 'karang_taruna' ? 'selected' : '' }}>Karang Taruna
                    </option>
                    <option value="lembaga_lain" {{ request('jenis') == 'lembaga_lain' ? 'selected' : '' }}>Lembaga Lain
                    </option>
                </select>
            </div>
            <div class="flex-1">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <p class="font-medium text-gray-900">{{ $aparatur->nama }}</p>
                                        @if ($aparatur->nip)
                                            <p class="text-sm text-gray-500">NIP: {{ $aparatur->nip }}</p>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <p class="text-gray-900">{{ $aparatur->jabatan }}</p>
                                @if ($aparatur->masa_jabatan)
                                    <p class="text-sm text-gray-500">{{ $aparatur->masa_jabatan }}</p>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                @switch($aparatur->jenis)
                                    @case('perangkat')
                                        <span
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.aparatur.edit', $aparatur) }}"
                                        class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.aparatur.destroy', $aparatur) }}" method="POST"
            ...
            @if ($aparaturs->hasPages())
                <div class="px-6 py-4 border-t border-gray-200">
                    {{ $aparaturs->withQueryString()->links() }}
                </div>
            @endif
        </div>
    @endsection
