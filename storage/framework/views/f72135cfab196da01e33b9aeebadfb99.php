                        ...
                        <?php $__currentLoopData = $potensis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $potensi): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="bg-white rounded-2xl shadow-sm p-6 hover:shadow-md transition" data-aos="fade-up"
                                ...
                                <div class="w-14 h-14 bg-primary-100 rounded-xl flex items-center justify-center mb-4">
                                    <?php if($potensi->kategori && $potensi->kategori->icon): ?>
                                        <i class="fas fa-<?php echo e($potensi->kategori->icon); ?> text-2xl text-primary-600"></i>
                                    <?php else: ?>
                                        <i class="fas fa-star text-2xl text-primary-600"></i>
                                    <?php endif; ?>
                                </div>
                                <h3 class="font-bold text-gray-800 mb-2"><?php echo e($potensi->nama); ?></h3>
                                <?php if($potensi->kategori): ?>
                                    <span
                                        class="text-xs text-primary-600 font-medium"><?php echo e($potensi->kategori->nama); ?></span>
                                <?php endif; ?>
                                <p class="text-gray-600 text-sm mt-2 line-clamp-3">
                                    <?php echo e(Str::limit(strip_tags($potensi->deskripsi), 100)); ?></p>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                </div>
